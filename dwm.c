...
void setup(void) {
  ...
  sigchld(0);
  debug_log = fopen("/tmp/dwm.log", "a");
  if (debug_log = fopen("/tmp/dwm.log", "a")) {
    fprintf(debug_log, "STARTUP --------------------");
    fclose(debug_log);
  }
  /* init screen */
...
void tile(Monitor *m) {
  ...
  for (i = 0, my = ty = gappx, c = nexttiled(m->clients); c;
    ...
    if (i < m->nmaster) {
      r = MIN(n, m->nmaster) - i;
      h = (m->wh - my) / r;
      resize(c, m->wx + gappx, m->wy + my, mw - (2 * c->bw) - gappx * 2,
             h - (2 * c->bw) - gappx, 0);
      my += HEIGHT(c) + gappx;
    } else {
      r = n - i;
      ...
      resize(c, m->wx + mw, m->wy + ty,
             m->ww - mw - (2 * c->bw) - gappx, h - (2 * c->bw) - gappx, 0);
      ty += HEIGHT(c) + gappx;
      /** ======= */
      /**   if(n > m->nmaster) */
      ...
      /**       resize(c, m->wx + mw + g, m->wy + ty, m->ww - mw - g -
